$(document).ready(function () {
  ...
  function fetchOrderDetails(orderId) {
    ...
    $.ajax({
      ...
      success: function (response) {
        if (response.status === "success") {
          ...
          // Get discount amount directly from DB
          var discountAmount = order.discount_amount
            ? parseFloat(order.discount_amount)
            : 0;
          var totalAmount = parseFloat(order.total_price); // total_price is already discounted
    ...
    <tr><td>Discount</td><td>€${discountAmount.toFixed(2)}</td></tr>
